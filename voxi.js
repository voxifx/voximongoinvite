    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
      if (!inviterData) {
        ...
        newInviter.save().then(x => {
          if (inviteChannel) inviteChannel.send(`${member} sunucuya katıldı! **Davet eden**: ${inviter.id == member.guild.id ? member.guild.name : inviter.tag} (**${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}** davet)`).catch(err => {});
        });
      } else {
        inviterData.fake++
      ...
      Database.findOne({ guildID: member.guild.id, userID: memberData.inviterID }, async (err, inviterData) => {
        if (!inviterData) {
         let newInviter = new Database({
          ...
          inviterData.save().then(x => {
            if (inviteChannel) inviteChannel.send(`\`${member.user.tag}\` sunucudan ayrıldı! ${inviter.tag ? `**Davet eden**: ${inviter.id == member.guild.id ? member.guild.name : inviter.tag} (**${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}** davet)` : `Davet eden kullanıcı bulunamadı!`}`).catch(err => {});
          });
        };
      });
    } else {
      if (inviteChannel) inviteChannel.send(`\`${member.user.tag}\` sunucudan ayrıldı! Davet eden kullanıcı bulunamadı!`).catch(err => {});
    };
  });
});
...
client.on("message", async message => {
  ...
  if (command === "davet" || command === "invites") {
  ...
  if (command === "topdavet" || command === "sıralama") {
...
//<a:deuslra_sohbet:849189612127518740> 
//<a:deuslra_yldzz:849189611930648576> 
//<a:deuslra_bilmem:849189611900370954> 
